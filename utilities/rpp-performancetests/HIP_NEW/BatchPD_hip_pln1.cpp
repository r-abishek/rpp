...
int main(int argc, char **argv)
{
    ...
    {
        printf("\nUsage: ./BatchPD_hip_pln1 <src1 folder> <src2 folder (place same as src1 folder for single image functionalities)> <u8 = 0 / f16 = 1 / f32 = 2 / u8->f16 = 3 / u8->f32 = 4 / i8 = 5 / u8->i8 = 6> <outputFormatToggle (pkd->pkd = 0 / pkd->pln = 1)> <case number = 0:81> <interp_type = 0:3> <verbosity = 0/1>\n");
        return -1;
    }
    ...
    for (int perfRunCount = 0; perfRunCount < 100; perfRunCount++)
    {
        ...
        case 21:
        {
            ...
            if (ip_bitDepth == 0)
                ...
                missingFuncFlag = 1; // rppi_resize_f16_pln1_batchPD_gpu(d_inputf16, srcSize, maxSize, d_outputf16, dstSize, maxDstSize, interpolation_type, outputFormatToggle, noOfImages, handle);
            else if (ip_bitDepth == 2)
                ...
                missingFuncFlag = 1; // rppi_resize_u8_f16_pln1_batchPD_gpu(d_input, srcSize, maxSize, d_outputf16, dstSize, maxDstSize, interpolation_type, outputFormatToggle, noOfImages, handle);
            else if (ip_bitDepth == 4)
        ...
        case 22:
        {
            ...
            if (ip_bitDepth == 0)
                rppi_resize_crop_u8_pln1_batchPD_gpu(d_input, srcSize, maxSize, d_output, dstSize, maxDstSize, x1, x2, y1, y2, interpolation_type, outputFormatToggle, noOfImages, handle);
            else if (ip_bitDepth == 1)
                ...
                rppi_resize_crop_f32_pln1_batchPD_gpu(d_inputf32, srcSize, maxSize, d_outputf32, dstSize, maxDstSize, x1, x2, y1, y2, interpolation_type, outputFormatToggle, noOfImages, handle);
            else if (ip_bitDepth == 3)
                missingFuncFlag = 1;
            else if (ip_bitDepth == 4)
                missingFuncFlag = 1;
            else if (ip_bitDepth == 5)
                rppi_resize_crop_i8_pln1_batchPD_gpu(d_inputi8, srcSize, maxSize, d_outputi8, dstSize, maxDstSize, x1, x2, y1, y2, interpolation_type, outputFormatToggle, noOfImages, handle);
            else if (ip_bitDepth == 6)
                missingFuncFlag = 1;
            else
                missingFuncFlag = 1;
