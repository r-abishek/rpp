...
__global__ void copy_pkd3_pkd3_tensor(T *srcPtr,
                                      uint2 srcStridesNH,
                                      U *dstPtr,
                                      uint2 dstStridesNH,
                                      uint2 maxDim)
...
template <typename T, typename U>
__global__ void copy_pln3_pln3_tensor(T *srcPtr,
                                      uint3 srcStridesNCH,
                                      U *dstPtr,
                                      uint3 dstStridesNCH,
                                      uint2 maxDim)
...
template <typename T, typename U>
__global__ void copy_pln1_pln1_tensor(T *srcPtr,
                                      uint2 srcStridesNH,
                                      U *dstPtr,
                                      uint2 dstStridesNH,
                                      uint2 maxDim)
                               ...
                               rpp::Handle& handle)
{
    ...
    if((srcDescPtr->c == 3) && (dstDescPtr->c == 3))
    {
        if ((srcDescPtr->layout == RpptLayout::NHWC) && (dstDescPtr->layout == RpptLayout::NHWC))
        {
            ...
            hipLaunchKernelGGL(copy_pkd3_pkd3_tensor,
        ...
        {
            hipLaunchKernelGGL(copy_pln3_pln3_tensor,
                               dim3(ceil((float)globalThreads_x/localThreads_x), ceil((float)globalThreads_y/localThreads_y), ceil((float)globalThreads_z/localThreads_z)),
                               dim3(localThreads_x, localThreads_y, localThreads_z),
                               0,
                               handle.GetStream(),
